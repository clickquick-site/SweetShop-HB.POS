...
app.post('/api/email', async (req, res) => {
  ...
  try {
    ...
    const info = await transporter.sendMail({
      from: `"HB-POS" <${config.emailSender}>`,
      to, subject,
      text: body || '',
      html: html || body || '',
    });
  ...
  try {
    ...
    const html = `
      <div dir="rtl" style="font-family:Arial,sans-serif;max-width:600px;margin:auto;padding:20px;">
        ...
        <p style="margin-top:20px;font-size:12px;color:#999;text-align:center;">HB-POS v8.1.0</p>
      </div>`;
    ...
    await transporter.sendMail({
      from: `"HB-POS" <${config.emailSender}>`,
      to,
      subject: `📊 التقرير اليومي ${report.date||''} — HB-POS`,
      html,
    });
...
app.listen(PORT, '0.0.0.0', () => {
  log('SERVER', `✅ HB-POS Server v8.1.0 يعمل على المنفذ ${PORT}`);
